...
// ── Budsjett-PDF style — multi-tool framework extension ──
// Kalles fra shared/core.js generateBudsjettPdf() via window.PDF_STYLES.budgetElite(b, meta).
// b   = { inntekt:[{lbl,amt},...], utgift:[{lbl,amt},...], sumInn, sumUt, overskudd, sparerate, aar }
// meta = { today, reportId }
(function(){
  'use strict';
  ...
  function buildBudgetEliteDoc(b, meta){
    var C = paletteB();
    var today = (meta && meta.today) || new Date().toLocaleDateString('nb-NO',{day:'numeric',month:'long',year:'numeric'});
    var reportId = (meta && meta.reportId) || ('HV-' + Date.now().toString(36).toUpperCase().slice(-6));

    var inntektListe = (b.inntekt || []);
    var utgiftListe  = (b.utgift  || []);
    var sumInn       = b.sumInn   || 0;
    var sumUt        = b.sumUt    || 0;
    var overskudd    = b.overskudd != null ? b.overskudd : (sumInn - sumUt);
    var sparerate    = b.sparerate != null ? b.sparerate : (sumInn > 0 ? (overskudd / sumInn) * 100 : 0);
    var aarligOverskudd = overskudd * 12;

    // Breakdown-tabell (inntekt + separator + utgift)
    var tableRows = [[
      { text:'POST', bold:true, color:C.ink2, fontSize:8, characterSpacing:1, margin:[0,4,0,4] },
      { text:'BELOP (KR/MND)', bold:true, color:C.ink2, fontSize:8, characterSpacing:1, alignment:'right', margin:[0,4,0,4] }
    ]];

    // Inntekter-header
    tableRows.push([
      { text:'INNTEKTER', bold:true, fontSize:9, color:C.accent, characterSpacing:1, margin:[0,8,0,4], colSpan:2 },
      {}
    ]);
    inntektListe.forEach(function(item, idx){
      tableRows.push([
        { text:item.lbl || item.name || '–', color:C.ink, margin:[0,5,0,5] },
        { text:fmt(item.amt != null ? item.amt : item.amount), alignment:'right', color:C.ink, margin:[0,5,0,5] }
      ]);
    });
    tableRows.push([
      { text:'Sum inntekter', bold:true, color:C.ink, margin:[0,6,0,6] },
      { text:fmt(sumInn), alignment:'right', bold:true, color:C.ink, margin:[0,6,0,6] }
    ]);

    // Separator
    tableRows.push([
      { canvas:[{ type:'line', x1:0, y1:3, x2:460, y2:3, lineWidth:0.5, lineColor:C.border }], colSpan:2, margin:[0,4,0,4] },
      {}
    ]);

    // Utgifter-header
    tableRows.push([
      { text:'UTGIFTER', bold:true, fontSize:9, color:C.accent, characterSpacing:1, margin:[0,8,0,4], colSpan:2 },
      {}
    ]);
    utgiftListe.forEach(function(item){
      tableRows.push([
        { text:item.lbl || item.name || '–', color:C.ink, margin:[0,5,0,5] },
        { text:fmt(item.amt != null ? item.amt : item.amount), alignment:'right', color:C.ink, margin:[0,5,0,5] }
      ]);
    ...
    tableRows.push([
      { text:'Sum utgifter', bold:true, color:C.ink, margin:[0,6,0,6] },
      { text:fmt(sumUt), alignment:'right', bold:true, color:C.ink, margin:[0,6,0,6] }
    ]);

    return {
      pageSize:'A4',
      pageMargins:[48, 80, 48, 60],
      info:{ title:'Budsjett-rapport — ' + today, author:'Hverdagsverktoy.com', subject:'Budsjettrapport' },
      background:function(currentPage, pageSize){
        var layers = [];
        if(C.pageBg) layers.push({ canvas:[{ type:'rect', x:0, y:0, w:pageSize.width, h:pageSize.height, color:C.pageBg }] });
        layers.push({ canvas:[{ type:'rect', x:0, y:0, w:pageSize.width, h:4, color:C.accent }] });
      ...
      header:function(curr){
        ...
        return { columns:[
          { text:'HVERDAGSVERKTOY', style:'brandMark', margin:[48,28,0,0] },
          { text:'BUDSJETT-RAPPORT · ' + reportId, style:'docMeta', alignment:'right', margin:[0,28,48,0] }
        ]};
      },
      content:[
        { text:'Budsjett-rapport', style:'h1', margin:[0,6,0,2] },
        { text:today, style:'sub', margin:[0,0,0,28] },

        // Summary-kort: 2×2 grid
        { table:{ widths:['*','*'], body:[[
          { stack:[
            { text:'MANEDLIG BALANSE', style:'heroLbl' },
            { text:fmt(overskudd), style:overskudd >= 0 ? 'heroVal' : 'heroValNeg' },
            { canvas:[{ type:'line', x1:0, y1:0, x2:30, y2:0, lineWidth:2, lineColor:C.accent }], margin:[0,4,0,8] },
            { text:fmt(aarligOverskudd) + ' per ar', style:'heroSub' }
          ], fillColor:C.surface2, margin:[22,22,22,22] },
          { stack:[
            { text:'SPARERATE', style:'heroLbl' },
            { text:sparerate.toFixed(1).replace('.',',') + ' %', style:'heroVal' },
            { canvas:[{ type:'line', x1:0, y1:0, x2:30, y2:0, lineWidth:2, lineColor:C.accent }], margin:[0,4,0,8] },
            { text:fmt(sumInn) + ' inn, ' + fmt(sumUt) + ' ut', style:'heroSub' }
          ], fillColor:C.surface2, margin:[22,22,22,22] }
        ]]}, layout:{
          hLineWidth:function(){ return 0; },
          vLineWidth:function(i){ return i===1 ? 0.5 : 0; },
        ...
        { text:'DETALJERT OVERSIKT', style:'sectionLbl' },
        { canvas:[{ type:'line', x1:0, y1:0, x2:499, y2:0, lineWidth:0.8, lineColor:C.accent }], margin:[0,2,0,12] },
        { table:{ widths:['*', 100], body:tableRows }, layout:{
          hLineWidth:function(i, n){
            if(i===0 || i===1) return 0.6;
            if(i===n.table.body.length) return 0.6;
            return 0;
          },
          hLineColor:function(){ return C.accent; },
          vLineWidth:function(){ return 0; },
      ...
      styles:{
        ...
        heroLbl:{ fontSize:8, bold:true, color:C.heroLbl, characterSpacing:1.8, margin:[0,0,0,10] },
        heroVal:{ fontSize:36, bold:true, color:C.ink, margin:[0,0,0,0] },
        heroValNeg:{ fontSize:36, bold:true, color:C.red, margin:[0,0,0,0] },
        heroSub:{ fontSize:11, color:C.ink, bold:true },
        heroSubMuted:{ fontSize:10, color:C.ink2, margin:[0,2,0,0] },
        note:{ fontSize:9, color:C.ink2, lineHeight:1.45 }
      },
  ...
  window.PDF_STYLES = window.PDF_STYLES || {};
  window.PDF_STYLES.budgetElite = buildBudgetEliteDoc;
})();
